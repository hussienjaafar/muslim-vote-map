...
export function RegionSearch({ statesData, districtsData, onSelect }: RegionSearchProps) {
          ...
          <CommandGroup heading={<span className="font-display">States</span>}>
            {stateOptions.map(s => (
              <CommandItem
                key={s.id}
                value={s.label}
                onSelect={() => handleSelect(s.id, 'state')}
              >
          ...
          <CommandGroup heading={<span className="font-display">Districts</span>}>
            {districtOptions.map(d => (
              <CommandItem
                key={d.id}
                value={d.label}
                onSelect={() => handleSelect(d.id, 'district')}
              >
                <div className="flex items-center justify-between w-full">
                  <span>{d.label}</span>
  ...
  return (
    <>
      <Button
        ...
        className="h-7 gap-1.5 text-xs text-muted-foreground font-normal px-2.5"
      >
