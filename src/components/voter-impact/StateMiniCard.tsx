...
export function StateMiniCard({ stateCode, color, value, onClick }: StateMiniCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-0.5 sm:gap-1 rounded-lg border border-border bg-[#1c1c1e]/80 backdrop-blur-[20px] p-1.5 sm:p-2 hover:bg-[#2c2c2e] transition-colors cursor-pointer min-h-[44px] min-w-[44px]"
      aria-label={`View ${stateCode}`}
    >
      <svg viewBox={viewBox} className="w-10 h-6 sm:w-16 sm:h-10">
        <path d={path} fill={color} stroke="hsl(var(--border))" strokeWidth="1" />
      </svg>
      <div className="flex items-center gap-0.5 sm:gap-1">
        <span className="text-[10px] sm:text-xs font-bold text-foreground font-display">{stateCode}</span>
        <span className="text-[10px] sm:text-xs text-muted-foreground hidden sm:inline">{value}</span>
      </div>
    </button>
  );
}
