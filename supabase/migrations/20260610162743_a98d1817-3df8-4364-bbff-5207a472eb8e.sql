ALTER TABLE public.sms_campaign_metrics ADD COLUMN IF NOT EXISTS refcode text;
...
BEGIN
  ...
  -- SMS Pass A: exact refcode-to-refcode match against broadcasts (high confidence).
  -- When several broadcasts share a refcode, attribute to the one sent closest to the donation date.
  UPDATE public.actblue_transactions t
  SET (attributed_channel, attributed_campaign) = (
        ...
        ORDER BY abs((t.transaction_date AT TIME ZONE 'America/New_York')::date - s.date) ASC NULLS LAST
        LIMIT 1
      ),
  ...
  -- SMS Pass B: legacy fallback (campaign_id / form_name contains), medium confidence.
  UPDATE public.actblue_transactions t
  SET (attributed_channel, attributed_campaign) = (
        SELECT 'sms', s.campaign_name
        FROM public.sms_campaign_metrics s
